...
To improve the UX of the application and make it feel more responsive.  We do not have to wait for the entire response before showing it to the user. 
...
To maintain state.  Sessions are what persist our paramaters across multiple requests and maintain state.  If we use globals, the parameters will be overwriten with each new session.
...
It seems to be adequate for document search but not at summarization
It is limited to the information provided in the context & in the document 
...
A web application that allows users to upload documents and ask questions about their content using a Retrieval Augmented Generation (RAG) system.
...
│ ├── package.json
│ └── public/
├── aimakerspace/ # Backend utilities
│ └── ...
├── api_app.py # FastAPI application
├── app_hf.py # Hugging Face specific app
├── requirements.txt # Python dependencies
├── Dockerfile # Docker configuration FastAPi/React
├── Dockerfile copy # Docker configuration Chainlit version (orig)
